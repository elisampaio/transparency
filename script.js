...
function write() {
  ...
  newText.style.color = 'rgb(' + value + ',' + value + ',' + value + ')';


  count++;
  // console.log(count);

  if (count >= -1 && count <= 665) {
    main.appendChild(newText);
  } else if (count >= 666 && count < 1332) {
    var lastitem = document.querySelector('.tp:last-of-type');
    main.removeChild(lastitem);
  } else if (count == 1332) {
    count = -1;
  } else {}
